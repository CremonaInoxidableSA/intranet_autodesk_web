...
export const GraficoBarrasDuracion = forwardRef<
  ChartJS<"bar"> | undefined,
  Propiedades
>(function GraficoBarrasDuracion({ sesiones, fechaDesde, fechaHasta }, ref) {
    ...
    const conjuntosInventor = Array.from(
      ...
      (_, indice) => ({
        ...
        data: dias.map((dia) => {
          ...
          return s ? s.horaFin - s.horaInicio : null
        }),
  ...
  return (
    <Bar
      ref={ref as React.ForwardedRef<ChartJS<"bar"> | undefined>}
      data={datosGrafico as ChartData<"bar", (number | null)[], string>}
      options={{
        responsive: true,
        plugins: {
          legend: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) => {
                const duracion = ctx.raw as number | null
                ...
                const nombre = etiqueta && etiqueta !== "" ? etiqueta : "Sesion"
                if (sesion) {
                ...
                return ` ${nombre}: ${duracion.toFixed(2)}h`
              },
            },
          },
        },
